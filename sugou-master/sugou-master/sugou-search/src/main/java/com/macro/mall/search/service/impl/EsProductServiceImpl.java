...
public class EsProductServiceImpl implements EsProductService {
    ...
    private EsProductRelatedInfo convertProductRelatedInfo(SearchHits<EsProduct> response) {
        ...
        //设置品牌
        //先取map集合里面的品牌名的聚合数据（brandName分类）
        ElasticsearchAggregation brandNames = esAggregationMap.get("brandName");
        //最后在brand里面找bukets组找到用数组存起来，
        ...
        //遍历整个buckets把里面的key也就是名字，用字符串表示
        ...
        List<String> productCategoryNamesList = new ArrayList<>();
        List<StringTermsBucket> productCategoryNamesBuckets = ((StringTermsAggregate)productCategoryNames.aggregation().getAggregate()._get()).buckets().array();
        for (StringTermsBucket productCategoryNamesBucket : productCategoryNamesBuckets) {
            productCategoryNamesList.add(productCategoryNamesBucket.key().stringValue());
        }
        productRelatedInfo.setProductCategoryNames(productCategoryNamesList);
