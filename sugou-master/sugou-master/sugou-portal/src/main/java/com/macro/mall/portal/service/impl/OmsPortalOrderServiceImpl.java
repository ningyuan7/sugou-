...
public class OmsPortalOrderServiceImpl implements OmsPortalOrderService {
    ...
    /**
     * 锁定下单商品的所有库存（使用Redisson看门狗自动续期）
     */
    private void lockStock(List<CartPromotionItem> cartPromotionItemList) {
        for (CartPromotionItem cartPromotionItem : cartPromotionItemList) {
            String lockKey = "lock:stock:" + cartPromotionItem.getProductSkuId();
            // Redisson tryLock(最大等待时间, 时间单位)，不指定leaseTime则启用看门狗自动续期
            boolean locked = redissonLockService.tryLock(lockKey, 5, TimeUnit.SECONDS);
            if (!locked) {
                throw new RuntimeException("系统繁忙，商品正在被其他用户购买请稍后重试");
            }
            try {
                //查找库存信息
